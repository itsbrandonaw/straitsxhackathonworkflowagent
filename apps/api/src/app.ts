...
import {
  ...
  WebsiteControlRequestSchema,
  WebsiteRejectRequestSchema,
  WebsiteSearchRequestSchema,
  toScoutRequest
} from "./website-bridge.js";

export type AppDependencies = {
  coordinator: ScoutCoordinator;
  events: EventHub;
  website?: WebsiteCallbackBridge;
};
...
export async function buildApp(dependencies: AppDependencies = createLocalDependencies()) {
  ...
  const capabilities = new LiveViewCapabilities(
    ...
    process.env.SCOUT_VIEWER_BASE_URL ?? "http://localhost:5173"
  );
  const website = dependencies.website
    ?? new WebsiteCallbackBridge(dependencies.coordinator, dependencies.events, capabilities);
  ...
  app.get("/health", async () => ({
    status: "ok",
    mode: "mock",
    purpose: "workflow-and-ui-testing-only",
    realAgentRuntime: "aws-bedrock-agentcore"
  }));
  ...
  app.post("/v1/runs/search", async (request, reply) => {
    const body = WebsiteSearchRequestSchema.parse(request.body);
    website.register(body);
    await dependencies.coordinator.start(toScoutRequest(body), body.activityId);
    return reply.code(202).send({ accepted: true, activityId: body.activityId });
  });

  app.post<{ Params: { activityId: string } }>("/v1/runs/:activityId/pause", async (request, reply) => {
    const body = WebsiteControlRequestSchema.parse(request.body);
    website.updateCallback(request.params.activityId, body.callback);
    await dependencies.coordinator.pause(request.params.activityId);
    return reply.code(202).send({ accepted: true, activityId: request.params.activityId, action: "pause" });
  });

  app.post<{ Params: { activityId: string } }>("/v1/runs/:activityId/resume", async (request, reply) => {
    const body = WebsiteControlRequestSchema.parse(request.body);
    website.updateCallback(request.params.activityId, body.callback);
    await dependencies.coordinator.resume(request.params.activityId);
    return reply.code(202).send({ accepted: true, activityId: request.params.activityId, action: "resume" });
  });

  app.post<{ Params: { activityId: string } }>("/v1/runs/:activityId/cancel", async (request, reply) => {
    const body = WebsiteControlRequestSchema.parse(request.body);
    website.updateCallback(request.params.activityId, body.callback);
    await dependencies.coordinator.cancel(request.params.activityId);
    capabilities.revokeActivity(request.params.activityId);
    return reply.code(202).send({ accepted: true, activityId: request.params.activityId, action: "cancel" });
  });

  app.post<{ Params: { activityId: string } }>("/v1/runs/:activityId/reject", async (request, reply) => {
    const body = WebsiteRejectRequestSchema.parse(request.body);
    website.updateCallback(request.params.activityId, body.callback);
    await dependencies.coordinator.reject(request.params.activityId, body.itemId, body.feedback);
    return reply.code(202).send({ accepted: true, activityId: request.params.activityId, action: "reject" });
  });

  app.post("/v1/live-view/connection", async (request, reply) => {
    const body = z.object({ capability: z.string().min(32).max(256) }).parse(request.body);
    try {
      return await capabilities.exchange(body.capability);
    } catch (error) {
